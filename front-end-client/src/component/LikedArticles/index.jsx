...
import { getAllLikeByUser } from '../../services/user';
import { unLike } from '../../services/like';
...
const LikedArticles = () => {
    ...
    // Hàm lấy danh sách like từ API
    const fetchLikes = async () => {
        try {
            setLoading(true);
            const response = await getAllLikeByUser();

            if (response?.status === 200) {
                const likeData = response.data;
                setArticles(likeData);
                setTotalPages(response.totalPages || 1);
                setTotalItems(response.totalItems || likeData.length);
            } else {
                toast.error(response?.message || 'Có lỗi xảy ra khi tải danh sách bài viết đã thích');
                setArticles([]);
            }
        } catch (error) {
            console.error('Error fetching likes:', error);
            toast.error('Có lỗi xảy ra khi tải danh sách bài viết đã thích');
            setArticles([]);
        } finally {
            setLoading(false);
        }
    ...
    // Hàm bỏ thích với confirm
    const handleUnlike = (articleId, articleTitle) => {
        Modal.confirm({
            title: 'Xác nhận bỏ thích',
            icon: <ExclamationCircleOutlined />,
            content: `Bạn có chắc chắn muốn bỏ thích bài viết "${articleTitle}"?`,
            okText: 'Xác nhận',
            cancelText: 'Hủy',
            okType: 'danger',
            ...
            onOk: async () => {
                try {
                    ...
                    const response = await unLike(user?.id, articleId);

                    if (response?.status === 200) {
                        toast.success('Đã bỏ thích bài viết thành công');
                        // Cập nhật lại danh sách sau khi bỏ thích
                        setArticles(prev => prev.filter(article => article.articlesId !== articleId));
                        setTotalItems(prev => prev - 1);
                    } else {
                        toast.error(response?.message || 'Có lỗi xảy ra khi bỏ thích bài viết');
                    }
                } catch (error) {
                    ...
                    <div className="flex items-center space-x-4 mb-4">
                        ...
                        <div>
                            ...
                            <p className="text-gray-600">
                                {totalItems} bài viết đã được thích
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="max-w-6xl mx-auto px-4 py-8">
                {articles.length === 0 ? (
                    <div className="text-center py-20">
                        ...
                        <p className="text-gray-500 mb-4">Bắt đầu thích những bài viết yêu thích để dễ dàng tìm lại</p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                            {currentArticles.map((article) => (
                                <article key={article.id} className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300 relative">
                                    ...
                                    {/* Unlike button */}
                                    <button
                                        onClick={() => handleUnlike(article.articlesId, article.nameArticles)}
                                    ...
                                    <div className="p-4">
                                        ...
                                        <div className="mt-2 pt-2 border-t border-gray-100">
                                            <div className="flex items-center text-xs text-gray-500">
                                                ...
                                                Đã thích {new Date(article.createAt).toLocaleDateString('vi-VN')}
                                            </div>
                                        </div>
                                    </div>
                                </article>
